...
# TOKEN_PATH_DRIVE = r"C:\\Users\\Nícolas Nasário\\OneDrive\\Cursos online\\Treinamento Python - Hashtag\\Códigos\\Nex.ai - Empresa\\Formus\\Envio Whats\\token_drive.json"
# TOKEN_PATH = r"C:\\Users\\Nícolas Nasário\\OneDrive\\Cursos online\\Treinamento Python - Hashtag\\Códigos\\Nex.ai - Empresa\\Formus\\Envio Whats\\token.json"
# CLIENT_SECRET_PATH = r"C:\\Users\\Nícolas Nasário\\OneDrive\\Cursos online\\Treinamento Python - Hashtag\\Códigos\\Nex.ai - Empresa\\Formus\\Envio Whats\\client_secret.json"
        ...
        messages=[
            {
                ...
                NÃO ALTERE O ANO / MÊS OU DIA QUE FOI INFORMADO NA MENSAGEM.
...
def formata_numero_waha(numero):
    """
    Garante que o número esteja no formato exigido pelo Waha: 554899999999@c.us
    """
    numero = str(numero).strip()
    # Remove caracteres não numéricos exceto @ e .
    numero_limpo = re.sub(r'[^\d@c.us]', '', numero)
    
    if "@c.us" not in numero_limpo:
        # Se vier apenas o número (ex: 55489...), adiciona o sufixo
        return f"{numero_limpo}@c.us"
    return numero_limpo


def simula_comportamento_humano(url_base, session, chat_id, headers, tamanho_mensagem=50):
    """
    Executa a sequência: Ler -> Digitar -> Esperar -> Parar -> Retornar
    """
    try:
        # 1. Enviar "Visto" (Send Seen)
        requests.post(
            f"{url_base}/api/sendSeen", 
            json={"session": session, "chatId": chat_id}, 
            headers=headers
        )

        # 2. Começar a "Digitar" (Start Typing)
        requests.post(
            f"{url_base}/api/startTyping", 
            json={"session": session, "chatId": chat_id}, 
            headers=headers
        )

        # 3. Calcular tempo de espera baseado no tamanho da mensagem
        # Uma pessoa média digita ~5 caracteres por segundo. 
        # Adicionamos um fator aleatório para parecer natural.
        tempo_digitando = (tamanho_mensagem / 5) + random.uniform(0.5, 1.5)
        
        # Limitamos para não ficar esperando 1 minuto em textos gigantes
        if tempo_digitando > 15: 
            tempo_digitando = random.uniform(10, 25)
        elif tempo_digitando < 2:
            tempo_digitando = random.uniform(2, 5) # Mínimo de 2 segundos
            
        print(f"[WAHA] Simulando digitação por {round(tempo_digitando, 2)} segundos...")
        time.sleep(tempo_digitando)

        # 4. Parar de "Digitar" (Stop Typing)
        requests.post(
            f"{url_base}/api/stopTyping", 
            json={"session": session, "chatId": chat_id}, 
            headers=headers
        )
        
    except Exception as e:
        print(f"[WAHA] Erro ao simular presença (não crítico): {e}")
...
def alternar_nono_digito(chat_id):
    """
    Recebe um chatId (ex: 5548999999999@c.us) e alterna entre ter ou não o nono dígito.
    Se tiver 13 dígitos (com o 9), remove. Se tiver 12 (sem o 9), adiciona.
    """
    # Remove o sufixo para analisar apenas os números
    numero_limpo = chat_id.replace('@c.us', '')
    
    # Verifica se é um número do Brasil (começa com 55)
    if not numero_limpo.startswith('55'):
        return chat_id # Retorna igual se não for BR

    novo_numero = numero_limpo

    # Lógica para alternar (Considerando formato 55 + DDD + NÚMERO)
    if len(numero_limpo) == 13: 
        # Tem 13 dígitos (55 + 2 DDD + 9 dígitos). Vamos remover o 9 (que está na posição 4, índice 4)
        # Ex: 55 48 9 9999... -> tira o '9' depois do 48
        novo_numero = numero_limpo[:4] + numero_limpo[5:]
        print(f"[WAHA] Tentativa de correção: Removendo 9º dígito -> {novo_numero}")
        
    elif len(numero_limpo) == 12:
        # Tem 12 dígitos (55 + 2 DDD + 8 dígitos). Vamos adicionar o 9
        novo_numero = numero_limpo[:4] + '9' + numero_limpo[4:]
        print(f"[WAHA] Tentativa de correção: Adicionando 9º dígito -> {novo_numero}")

    return f"{novo_numero}@c.us"
...
def send_whatsapp_message(number, text):
    ...
    # Formata o número inicial
    chat_id = formata_numero_waha(number)

    # Executa a simulação humana antes de enviar (apenas na primeira tentativa para não demorar demais)
    simula_comportamento_humano(waha_url, waha_session, chat_id, headers, tamanho_mensagem=len(text))

    print(f"[WAHA] Enviando Texto para: {chat_id}")

    # Função interna para realizar a requisição
    def tentar_requisicao(target_chat_id):
        payload = {
    ...
    try:
        # 1ª Tentativa
        response = tentar_requisicao(chat_id)
        
        # Se der erro 500, tentamos corrigir o número
        if response.status_code == 500:
            print(f"[WAHA] Erro 500 detectado. Tentando alternar nono dígito...")
            novo_chat_id = alternar_nono_digito(chat_id)
            
            if novo_chat_id != chat_id:
                # 2ª Tentativa com o número corrigido
                response = tentar_requisicao(novo_chat_id)
                if response.status_code == 200:
                    print(f"[WAHA] Sucesso na segunda tentativa com número: {novo_chat_id}")

        # Verifica se deu erro em definitivo (na 1ª ou na 2ª tentativa)
        response.raise_for_status()
        print(f"[WAHA] Sucesso: {response.status_code}")
        return True, ""

    except requests.exceptions.RequestException as e:
        print(f"[WAHA] Erro ao enviar mensagem: {e}")
        return False, str(e)
...
def send_whatsapp_media(number, caminho_arquivo, caption=None):
    ...
    # Executa a simulação humana
    tamanho_simulacao = len(caption) if caption else 20
    simula_comportamento_humano(waha_url, waha_session, chat_id, headers, tamanho_mensagem=tamanho_simulacao)

    # Detecta mimetype e prepara arquivo
    mimetype, _ = mimetypes.guess_type(caminho_arquivo)
    if mimetype is None:
    ...
    if caption is None:
        caption = os.path.splitext(nome_arquivo)[0]

    print(f"[WAHA] Enviando Arquivo ({mimetype}) para: {chat_id}")

    # Função interna para realizar a requisição
    def tentar_requisicao_media(target_chat_id):
        payload = {
            "session": waha_session,
            "chatId": target_chat_id,
            "caption": caption,
    ...
    try:
        # 1ª Tentativa
        response = tentar_requisicao_media(chat_id)

        # Se der erro 500, tentamos corrigir o número
        if response.status_code == 500:
            print(f"[WAHA] Erro 500 no envio de mídia. Tentando alternar nono dígito...")
            novo_chat_id = alternar_nono_digito(chat_id)
            
            if novo_chat_id != chat_id:
                # 2ª Tentativa
                response = tentar_requisicao_media(novo_chat_id)
                if response.status_code == 200:
                    print(f"[WAHA] Mídia enviada com sucesso na segunda tentativa: {novo_chat_id}")

        response.raise_for_status()
        print(f"[WAHA] Arquivo enviado: {response.status_code}")
        return True, ""

    except requests.exceptions.RequestException as e:
        print(f"[WAHA] Erro ao enviar mídia: {e}")
        
        destinatarios_email = ['nicolaspn09@gmail.com']
        assunto_email = "ERRO Envio Arquivos Whats - Waha"
        mensagem_erro = f"Erro ao enviar arquivos no Waha!<br>Erro: {e}"
        envia_email(mensagem_email=mensagem_erro, destinatarios_email=destinatarios_email, assunto_email=assunto_email)
...
def main():
    try:
        ...
        if contador < 40 and contador_atual <= 5:
