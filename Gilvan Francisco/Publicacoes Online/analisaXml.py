...
def criar_xml_saida(registros_incluidos, encoding='ISO-8859-1'):
    root = ET.Element("root")

    for record in registros_incluidos:
        row = ET.SubElement(root, "row")
        for tag, valor in record.items():
            elem = ET.SubElement(row, tag)
            elem.text = ET.CDATA(valor)

    xml_str = ET.tostring(root, encoding=encoding, xml_declaration=True, pretty_print=True)
...
if __name__ == "__main__":
    ...
    for record in records:
        ...
        # if data_atual in data_publicacao and len(tabela_sql) == 0:
            ...
            if "</think>" in str(texto1):
                texto1_dividido = texto1.split("</think>")
                texto1 = texto1_dividido[1]
                texto1 = texto1.strip()

            informar_dados_banco(data, data_publicacao, processo, data_publicacao, codigo, cidade, orgao, processo, texto, texto1, "Não excluído", linha) # DESCOMENTAR

            registros_incluidos.append(record)
            ...
            for texto_extraido_tabela, linha_tabela_sql in tabela_sql:                
                ...
                if "</think>" in str(texto2):
                    texto2_dividido = texto2.split("</think>")
                    texto2 = texto2_dividido[1]
                    texto2 = texto2.strip()
                ...
                if "sim" in str(retorno_gemini).lower():
                    if "texto1" in str(retorno_gemini).lower() or "texto2" in str(retorno_gemini).lower():
                        print("Aqui nesse caso não vamos informar no XML")

                        informar_dados_banco(data, data_publicacao, processo, data_publicacao, codigo, cidade, orgao, processo, texto, texto1, "Excluído", linha) # DESCOMENTAR

                        break

                    elif "empresas" in str(retorno_gemini).lower():
                        print("Aqui nesse caso vamos informar no XML")
    ...
    xml_gerado = criar_xml_saida(registros_incluidos)

    with open(rf"""C:\Users\nicol\OneDrive\Cursos online\Treinamento Python - Hashtag\Códigos\Nex.ai - Empresa\Gilvan Francisco\Publicacoes Online\XML\{datetime.now().strftime("%Y-%m-%d")}.xml""", "w", encoding="ISO-8859-1") as f:
        f.write(xml_gerado)